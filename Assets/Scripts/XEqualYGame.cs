...
public class XEqualYGame : MonoBehaviour
{
    ...
    void EndGame()
    {
        ...
        {
            endText.GetComponent<Text>().text += "第 " + (i + 1).ToString() + " 轮 : ";;
            if(scores[i] == 0)
            {
                endText.GetComponent<Text>().text += "错误 | " + " 时间 : " + times[i].ToString("0.00") + "\n";
            }
            else
            {
                endText.GetComponent<Text>().text += "正确 | " + "时间 : " + times[i].ToString("0.00") + "\n";
            }
        }
